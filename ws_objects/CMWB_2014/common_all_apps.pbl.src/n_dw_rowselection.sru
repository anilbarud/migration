$PBExportHeader$n_dw_rowselection.sru
$PBExportComments$PFC DataWindow Row Selection service
forward
global type n_dw_rowselection from nonvisualobject
end type
end forward
...
global n_dw_rowselection n_dw_rowselection
...
public function integer of_selectrow (long al_row, boolean ab_select)
...
This.of_SelectRow ( al_row,  NOT (idw_Requestor.IsSelected(al_row)) ) 
...
public function integer of_selectrow (long al_row, boolean ab_select);
IF al_row > 0 Then
	If idw_Requestor.event ue_rowselecting(al_row) = -1 THEN 
		RETURN -1
	END IF
END IF

	
idw_requestor.SelectRow(al_row,ab_select)
...
on n_dw_rowselection.create
call super::create
TriggerEvent( this, "constructor" )
end on

on n_dw_rowselection.destroy
