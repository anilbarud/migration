$PBExportHeader$w_converted_manuals.srw
forward
global type w_converted_manuals from w_a_report
end type
type dw_criteria from u_dw_online within w_converted_manuals
end type
type cb_retrieve from commandbutton within w_converted_manuals
end type
type gb_1 from groupbox within w_converted_manuals
end type
end forward

global type w_converted_manuals from w_a_report
string title = "Converted Manual Transactions"
dw_criteria dw_criteria
cb_retrieve cb_retrieve
gb_1 gb_1
end type
global w_converted_manuals w_converted_manuals
...
on w_converted_manuals.destroy
call super::destroy
...
event open;call super::open;id_current_date = Date(f_server_datetime())
...
type dw_report from w_a_report`dw_report within w_converted_manuals
integer x = 0
integer y = 352
integer width = 2688
integer height = 2160
integer taborder = 30
string dataobject = "d_ntr_conversion_details"
...
IF IsNull(ldt_from_date) Then
	...
	RETURN -1
ELSEIF Date(ldt_from_date) < Date('2005-09-01') THEN
	MessageBox('From date','From dates earlier than "2005-09-01" are not valid.')
	dw_criteria.SetColumn("from_date")
	dw_criteria.SetFocus()
	RETURN -1
...
IF IsNUll(ldt_to_date) Then
	...
	dw_criteria.SetFocus()
	RETURN -1
ELSEIF Date(ldt_to_date) < Date('2005-09-01') THEN
	MessageBox('To date','To dates earlier than "2005-09-01" are not valid.')
	dw_criteria.SetColumn("to_date")
	dw_criteria.SetFocus()
	RETURN -1
...
If ll_rows < 0 Then SignalError(-666,'Error retrieving rows')
...
string text = "Retrieve all conversions within the specified date range:"
end type
