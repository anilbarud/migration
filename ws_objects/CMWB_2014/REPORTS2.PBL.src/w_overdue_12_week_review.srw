$PBExportHeader$w_overdue_12_week_review.srw
$PBExportComments$Window to produce report of all claims overdue for their 12 week review for a user specified region.
forward
global type w_overdue_12_week_review from w_a_report
end type
type cb_ok from commandbutton within w_overdue_12_week_review
end type
type dw_select_region from u_dw_online within w_overdue_12_week_review
end type
type gb_1 from groupbox within w_overdue_12_week_review
end type
end forward

global type w_overdue_12_week_review from w_a_report
integer width = 3630
string title = "Claims Overdue for 12 Week Review"
cb_ok cb_ok
dw_select_region dw_select_region
gb_1 gb_1
end type
global w_overdue_12_week_review w_overdue_12_week_review
...
on w_overdue_12_week_review.create
int iCurrent
call super::create
...
on w_overdue_12_week_review.destroy
call super::destroy
...
type dw_report from w_a_report`dw_report within w_overdue_12_week_review
integer width = 3506
integer taborder = 40
string dataobject = "d_overdue_12_week_review"
...
/*	Determine what the cutoff date is for claims to be overdue for 12 week review (They are overdue
	if the accident_recurrence_date < (today - 12 weeks), so we send the date value of (today - 12 weeks)
	to the datawindow.
*/
//	adt_due_date = DateTime(RelativeDate(Today(),-84))
	adt_due_date = DateTime(RelativeDate(Date(f_server_datetime()),-84))
/*	Retrieve the report.
*/
	ll_numrows = dw_report.Retrieve(as_admin_region_code,adt_due_date)
	SQLCA.nf_handle_error("w_overdue_12_week_review","dw_report","cb_ok")
	IF ll_numrows <= 0 then
		MessageBox("Claims Overdue for 12 Week Review","No data found to satisfy request.",Exclamation!)
		RETURN
	END IF
