$PBExportHeader$w_charge_to_group_reserve.srw
$PBExportComments$Window to retrieve the report of all claims to be cost allocated to a reserve or group during the user specified time period.
forward
global type w_charge_to_group_reserve from w_a_report
end type
type cb_ok from commandbutton within w_charge_to_group_reserve
end type
type gb_1 from groupbox within w_charge_to_group_reserve
end type
type dw_enter_date_range from u_dw_online within w_charge_to_group_reserve
end type
end forward

global type w_charge_to_group_reserve from w_a_report
boolean TitleBar=true
string Title="Claims Cost Allocated to Group, 2nd Injury or Prior Years"
cb_ok cb_ok
gb_1 gb_1
dw_enter_date_range dw_enter_date_range
end type
global w_charge_to_group_reserve w_charge_to_group_reserve
...
on w_charge_to_group_reserve.create
int iCurrent
...
on w_charge_to_group_reserve.destroy
...
type dw_report from w_a_report`dw_report within w_charge_to_group_reserve
int Y=493
int TabOrder=40
string DataObject="d_charge_to_group"
...
/*	Validate the dates and set the from date to the next day at midnight to ensure that all transactions
	...
	created on the last day wont get selected because of the time. As well, also check that the date is
...
*/
	...
	SQLCA.nf_handle_error("w_charge_to_group_reserve","dw_report","cb_ok")
	IF ll_numrows <= 0 then
		MessageBox("Cost Allocated to Group/Reserve Report","No data found to satisfy request")
	END IF
